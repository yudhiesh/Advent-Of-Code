...
def validPasswords(strings):
    splitStrings = strings.splitlines()
    count = 0
    for strings in splitStrings:
        splitByColon = strings.split(":")
        letter = splitByColon[0][-1]
        splitByDash = splitByColon[0].split("-")
        start = int(splitByDash[0])
        splitBySpace = splitByDash[1].split(" ")
        end = int(splitBySpace[0])
        password = str(splitByColon[1])
...
def validPasswords2(strings):
    ...
    for strings in splitStrings:
        ...
        password = list(splitByColon[1])
        passwordStart = password[start]
        passwordEnd = password[end]
        letterCount = 0
        if passwordStart == letter:
            letterCount += 1
        if passwordEnd == letter:
            letterCount += 1
        if letterCount == 1:
            count += 1
            letterCount = 0
        elif letterCount == 0 or letterCount == 2:
            letterCount = 0
